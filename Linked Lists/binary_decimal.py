'''
Converting the values of a linkedlist from binary to decimal, where each node contains 1 or 0
...
1.1 Initialize num to 0. This variable will hold the resultant decimal number.

1.2 Initialize current to self.head. This variable will be used to traverse the linked list.

While current is not None

2.1 Update num as num * 2 + current.value

This step effectively shifts the binary digits left by one place (equivalent to multiplying by 2) and then adds the current binary digit.

2.2 Move to the next node by setting current = current.next.

After the end of the loop, num will hold the decimal number equivalent of the binary number stored in the linked list.
...
class LinkedList:
    ...
    #Solution binary to decimal problem
